...
#   - dose-response time-course assumes univariate likelihood
#   - under 3 different data scenarios
#   - estimates are saved in ./sim_results/univariate_ll/
#
################################################################################
...
if(export) saveRDS(NMAList, paste0(loc,"/sim_results/univariate_ll/NMAList.rds"))
...
if(export) saveRDS(timeNMAList, paste0(loc, "/sim_results/univariate_ll/timeNMAList.rds"))
...
dosetimeNMAList = future_lapply(future.seed = T, X = seq_along(dataLst), FUN = function(i){
  ...
  exp.NMA.emax.log <- brm(bf(y | se(se) ~ (MUemaxT + (emaxA*dA + emaxB*dB + emaxC*dC)*dose/(exp( ed50A*dA + ed50B*dB + ed50C*dC ) + dose))*( 1 - exp( -exp( MUkT + sA*dA*log(dose + 1) + sB*dB*log(dose + 1) + sC*dC*log(dose + 1) )*week ) ),
...
if(export) saveRDS(dosetimeNMAList, paste0(loc, "/sim_results/univariate_ll/dosetimeNMAList.rds"))
...
if(export) saveRDS(dosetimeNMAList2, paste0(loc, "/sim_results/univariate_ll/dosetimeNMAList2.rds"))
...
if(export) saveRDS(dosetimeNMAList3, paste0(loc, "/sim_results/univariate_ll/dosetimeNMAList3.rds"))