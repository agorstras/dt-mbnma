...
data {
  ...
  int<lower=1> K_emaxLira;  // number of population-level effects
  matrix[N, K_emaxLira] X_emaxLira;  // population-level design matrix
  int<lower=1> K_emaxSema;  // number of population-level effects
  matrix[N, K_emaxSema] X_emaxSema;  // population-level design matrix
  int<lower=1> K_ed50Lira;  // number of population-level effects
  matrix[N, K_ed50Lira] X_ed50Lira;  // population-level design matrix
  int<lower=1> K_ed50Sema;  // number of population-level effects
  matrix[N, K_ed50Sema] X_ed50Sema;  // population-level design matrix
...
parameters {
  ...
  vector[K_emaxLira] b_emaxLira;  // regression coefficients
  vector[K_emaxSema] b_emaxSema;  // regression coefficients
  vector[K_ed50Lira] b_ed50Lira;  // regression coefficients
  vector[K_ed50Sema] b_ed50Sema;  // regression coefficients
...
transformed parameters {
  ...
  r_1_MUemaxT_1 = r_1[, 1];
  r_1_kT_2 = r_1[, 2];
  lprior += normal_lpdf(b_MUemaxT[1] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[2] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[3] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[4] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[5] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[6] | -2.5, 100);
  lprior += normal_lpdf(b_MUemaxT[7] | -2.5, 100);
  lprior += normal_lpdf(b_emaxLira | -15, 100);
  lprior += normal_lpdf(b_emaxSema | -25, 100);
  lprior += normal_lpdf(b_ed50Lira | 1.5, 100);
  lprior += normal_lpdf(b_ed50Sema | 0.75, 100);
...
model {
  ...
  if (!prior_only) {
    ...
    // initialize linear predictor term
    vector[N] nlp_emaxLira = rep_vector(0.0, N);
    // initialize linear predictor term
    vector[N] nlp_emaxSema = rep_vector(0.0, N);
    // initialize linear predictor term
    vector[N] nlp_ed50Lira = rep_vector(0.0, N);
    // initialize linear predictor term
    vector[N] nlp_ed50Sema = rep_vector(0.0, N);
    // initialize linear predictor term
    vector[N] nlp_kT = rep_vector(0.0, N);
    // initialize non-linear predictor term
    vector[N] mu;
    nlp_MUemaxT += X_MUemaxT * b_MUemaxT;
    nlp_emaxLira += X_emaxLira * b_emaxLira;
    nlp_emaxSema += X_emaxSema * b_emaxSema;
    nlp_ed50Lira += X_ed50Lira * b_ed50Lira;
    nlp_ed50Sema += X_ed50Sema * b_ed50Sema;
    nlp_kT += X_kT * b_kT;
    for (n in 1:N) {
    ...
    for (n in 1:N) {
      ...
      mu[n] = ((nlp_MUemaxT[n] + (nlp_emaxLira[n] * C_1[n] + nlp_emaxSema[n] * C_2[n]) * C_3[n] / (exp(nlp_ed50Lira[n] * C_1[n] + nlp_ed50Sema[n] * C_2[n]) + C_3[n])) * (1 - exp( - exp(nlp_kT[n]) * C_4[n])));
    }
